...
def fetch_udisc_leaderboard():
    ...
    if os.path.exists(csv_path):
        try:
            ...
            with open(csv_path, newline='', encoding='utf-8') as f:
                ...
                for row in reader:
                    user = row.get('Player') or row.get('Username') or row.get('user') or 'Unknown'
                    try:
                        score = int(row.get('Total') or row.get('Score') or 999)
                    except:
                        continue
                    date = row.get('Date') or ''
                    if user not in best or score < best[user]['score']:
                        best[user] = {'username': user, 'score': score, 'date': date}
            sorted_best = sorted(best.values(), key=lambda x: x['score'])[:100]
    ...
    if not leaderboard:
        leaderboard = [
            ...
            {"rank":5,"username":"Toni Luoma-aho","score":40,"date":"29.4.2026"},
        ]
    return leaderboard
...
def main():
    ...
    print(f"Updated leaderboard {len(result['udisc_leaderboard'])}")
